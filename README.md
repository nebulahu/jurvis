...
一个从零开始构建的个人 AI 助手 MVP。当前版本已经完成文字交互核心闭环、Windows 语音交互、流式分句朗读、播放打断、唤醒词和自动录音端点检测。
...
- 显式开启后可只读列出允许窗口、识别前台窗口并读取受限 UIA 控件树
- 经确认后写入文件或保存长期记忆
...
运行 `jarvis` 后输入 `/voice`。语音模式会显示录音转写结果，再交给同一个 Agent 处理，因此权限确认、工具审计和长期记忆规则与文字模式完全一致。回答生成时会按完整句子进入后台播放队列；按 Esc 会清空待播放内容并停止当前朗读，然后可以立即开始下一轮录音。Esc 当前只中断播放，不取消已经发出的模型请求。设置 `JARVIS_TTS_ENABLED=false` 可以只录音和转写，不朗读。
...
工具可以在执行前根据实际参数把基础风险等级向上提升，但不能降低。桌面动作的运行时分类基础已建立：普通本地动作最低为 L2，发送、提交、上传、发布和删除类目标提升为 L3，密码、支付、认证及关键安全设置归为 L4。确认框与审计使用脱敏参数预览，真实执行参数不会因此被改写。
...
当前能力只覆盖“查找并打开应用”，尚不包含 Codex Computer Use 的屏幕截图、窗口定位、鼠标点击和键盘输入。

Computer Use 基础配置已经预留，但默认关闭，且当前尚未装配任何应用内控制工具：
...
其中快照节点上限、树深度和单项文本长度用于约束以后暴露给模型的 UI Automation 内容，快照有效期用于防止操作过期界面，操作超时用于限制单次桌面后端调用。保持控制开关为 `false` 不影响现有的应用启动工具。
...
安装桌面可选依赖并将 `JARVIS_DESKTOP_CONTROL_ENABLED` 显式设为 `true` 后，模型会获得 `list_windows`、`get_active_window` 和 `inspect_window` 三个 L1 只读工具，以及 `focus_window`、`invoke_element` 和 `select_element` 三个需要确认的语义动作工具。元素动作只能使用当前允许窗口和未过期快照中的引用，执行后旧快照立即失效。文字、语音和唤醒词入口使用同一个 Agent 和工具注册表，因此能力与审计规则一致：

```powershell
python -m pip install -e ".[desktop]"
```

桌面控制开关默认关闭。窗口聚焦和 UIA Invoke/SelectionItem 已实现；普通文本输入、任意按键、滚动、截图和坐标点击仍未实现。首版不会注册任意坐标点击工具。
